...
public class MemberUserController {
    ...
    @Operation(summary = "更新是否为导师")
    @PreAuthorize("@ss.hasPermission('member:user:update')")
    public CommonResult<Boolean> updateTeacher(@RequestBody @Valid MemberUserUpdateTeacherReqVO reqVO) {
        memberUserService.updateIzteacher(reqVO.getId(), reqVO.getIzteacher());
        return success(true);
    }
    ...
    public CommonResult<Boolean> forceUpdateUserPassword(@RequestBody @Valid MemberUserDO reqVO) {
    ...
    public void exportYwAuthLevelExcel(@Valid MemberUserPageReqVO pageReqVO,
                                       HttpServletResponse response) throws IOException {
        pageReqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        List<MemberUserDO> list = memberUserService.getUserPage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "评审申请.xls", "数据", MemberUserExportRespVO.class,
                BeanUtils.toBean(list,MemberUserExportRespVO.class));
    }
}
