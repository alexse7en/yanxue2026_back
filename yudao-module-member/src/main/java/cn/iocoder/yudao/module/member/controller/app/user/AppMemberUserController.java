...
public class AppMemberUserController {
    ...
    public CommonResult<AppMemberUserInfoRespVO> getUserInfo() {
        MemberUserDO bean = userService.getUser(getLoginUserId());
        MemberLevelDO level = levelService.getLevel(bean.getLevelId());
        AppMemberUserInfoRespVO appMemberUserInfoRespVO=MemberUserConvert.INSTANCE.convert(bean, level);
        appMemberUserInfoRespVO.setStatus( bean.getStatus() );
        appMemberUserInfoRespVO.setArea( bean.getArea() );
        appMemberUserInfoRespVO.setMark( bean.getMark() );
        appMemberUserInfoRespVO.setIdcard( bean.getIdcard() );
        appMemberUserInfoRespVO.setOrgName( bean.getOrgName() );
        appMemberUserInfoRespVO.setOrgSchool( bean.getOrgSchool() );
        appMemberUserInfoRespVO.setOrgProfession( bean.getOrgProfession() );
    ...
    @PutMapping("/addUserDetail")
    @Operation(summary = "重置密码", description = "用户忘记密码时使用")
    @PermitAll
    public CommonResult<Boolean> addUserDetail(@RequestBody @Valid MemberUserDO reqVO) {
        userService.addUserDetail(reqVO);
        return success(true);
    }

    @PutMapping("/addUserRenzhen")
    @Operation(summary = "重置密码", description = "用户忘记密码时使用")
    @PermitAll
    public CommonResult<Boolean> addUserRenzhen(@RequestBody @Valid MemberUserDO reqVO) throws TencentCloudSDKException {
        userService.addUserRenzhen(reqVO);
        return success(true);
    }
