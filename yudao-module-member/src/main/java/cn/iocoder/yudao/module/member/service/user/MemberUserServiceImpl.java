...
public class MemberUserServiceImpl implements MemberUserService {
    ...
    public void updateUser(MemberUserUpdateReqVO updateReqVO) {
        ...
        updateObj.setOrgName(updateReqVO.getOrgName());
        updateObj.setOrgProfession(updateReqVO.getOrgProfession());
        updateObj.setOrgSchool(updateReqVO.getOrgSchool());
        updateObj.setLevelId(updateReqVO.getLevelId());
        memberUserMapper.updateById(updateObj);
    }
    ...
    @Override
    public boolean addUserDetail(MemberUserDO reqVO) {
        if (reqVO==null ) {
            throw exception(USER_NOT_EXISTS, reqVO);
        }
        ...
        int i=memberUserMapper.addUserDetail(memberId,reqVO.getOrgSchool(),reqVO.getOrgProfession(),reqVO.getMark(),reqVO.getArea());
        if(user.getStatus()==null || user.getStatus()==0){
            memberUserMapper.addUserRenzhenStatus(user.getId(),1L);
        }
        if(i>0){
            return true;
        }else{
            throw exception(USER_DETAIL_FAIL, user);
        }
    }
    ...
    String SecretKey;
    @Override
    public boolean addUserRenzhen(MemberUserDO reqVO) throws TencentCloudSDKException {
        if (reqVO==null ) {
            throw exception(USER_NOT_EXISTS, reqVO);
        }
        Long memberId= SecurityFrameworkUtils.getLoginUserId();
        ...
        int i=memberUserMapper.addUserRenzhen(memberId,reqVO.getName(),reqVO.getIdcard());
        if(i>0){
            //这里要去做阿里云的身份证认证
            Boolean faceResult=TencentFaceIdUtil.verifyIdCard(SecretId,SecretKey,reqVO.getName(),reqVO.getIdcard());
            if(faceResult){
                memberUserMapper.addUserRenzhenStatus(memberId,10L);
                return true;
            }else{
                throw exception(USER_RENZHEN_FAIL, memberId);
            }
        }else{
            throw exception(USER_DETAIL_FAIL, memberId);
        }
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateIzteacher(Long id, Integer izteacher) {
        MemberUserDO user = memberUserMapper.selectById(id);
        if (user == null) {
            // 按你项目里已有的异常码来，这里示例
            throw exception(USER_NOT_EXISTS);
