...
public class YwCertStudentExcelParser {
    ...
    public List<YwCertStudentDO> parse(String filePath) throws Exception {
        try (InputStream inputStream = openStream(filePath);
            ...
            List<YwCertStudentDO> result = new ArrayList<>();
            for (int i = headerRowIndex + 1; i <= sheet.getLastRowNum(); i++) {
                Row row = sheet.getRow(i);
                if (row == null || isEmptyRow(row)) {
                    continue;
                }
                YwCertStudentDO item = new YwCertStudentDO();
    ...
    private String resolveField(String header) {
        ...
        if (containsAny(header, "学生姓名", "姓名")) {
            return "studentName";
        }
        if (containsAny(header, "身份证号", "身份证号码", "证件号码")) {
            return "idCard";
        }
        if (containsAny(header, "学校名称", "学校")) {
            return "schoolName";
        }
        if (containsAny(header, "班级", "班级名称")) {
            return "className";
        }
        if (containsAny(header, "课程名称", "研学课程", "课程主题", "课程")) {
            return "courseName";
        }
        if (containsAny(header, "学习课时", "课时")) {
            return "courseHours";
        }
        if (containsAny(header, "课程实施方", "实施方")) {
            return "courseProvider";
        }
        if (containsAny(header, "落证时间", "证书日期", "发证日期", "日期")) {
            return "certDate";
        }
        if (containsAny(header, "落章单位", "盖章单位", "发证单位")) {
            return "stampUnit";
        }
        return null;
    }
