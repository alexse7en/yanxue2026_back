...
public class NoticeController {
    ...
    @PostMapping("/sendSingleNotifyToMember")
    @Operation(summary = "推送通知公告", description = "只发送给 websocket 连接在线的用户")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('system:notice:update')")
    public CommonResult<Long> sendSingleNotifyToMember() {
        Map<String, Object> templateParams = new HashMap<>();
        templateParams.put("key1", "奥特曼");
        templateParams.put("key2", "变身");
        Long result=sendService.sendSingleNotifyToMember(253L, "zp_success",templateParams);
