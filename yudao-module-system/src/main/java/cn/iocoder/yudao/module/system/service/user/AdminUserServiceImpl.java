...
public class AdminUserServiceImpl implements AdminUserService {
    ...
    public Long registerUser(AuthRegisterReqVO registerReqVO) {
        ...
        userMapper.insert(user);
        permissionService.assignUserRole(user.getId(), Collections.singleton(158L));
        return user.getId();
    }
    ...
    public PageResult<AdminUserDO> getUserPage(UserPageReqVO reqVO) {
        ...
        // 1) 原分页
        PageResult<AdminUserDO> page = userMapper.selectPage(reqVO, getDeptCondition(reqVO.getDeptId()), userIds);

        // 2) 收集 username
        Set<String> invitedNames = page.getList().stream()
        ...
        // 3) 一次性构建统计 Map（final / 有效 final）
        final Map<String, Integer> cntMap = invitedNames.isEmpty()
                ? Collections.emptyMap()
                : userMapper.selectContribCountByInvitedNames(invitedNames).stream()
                .collect(Collectors.toMap(
                        AdminUserMapper.OrgContribCount::getIzinvited,
                        AdminUserMapper.OrgContribCount::getCnt,
                        (a, b) -> a // 保险：有重复键时取第一个
                ));

        // 4) 回填
        page.getList().forEach(u -> {
            String key = Optional.ofNullable(u.getUsername()).orElse("").trim();
            u.setContributionCount(cntMap.getOrDefault(key, 0));
        });
    ...
    @Override
    public void updateUserIzvip(Long id, Integer izvip) {
        AdminUserDO user = userMapper.selectById(id);
        if (user == null) {
            throw ServiceExceptionUtil.exception(USER_NOT_EXISTS);
        }
        int val = (izvip != null && Objects.equals(izvip, 1)) ? 1 : 0;
        userMapper.update(null, new LambdaUpdateWrapper<AdminUserDO>()
                .eq(AdminUserDO::getId, id)
                .set(AdminUserDO::getIzvip, val));
    }

    @Override
    public void updateUserHonor(Long id, String honor) {
        AdminUserDO user = userMapper.selectById(id);
        if (user == null) {
            throw ServiceExceptionUtil.exception(USER_NOT_EXISTS);
        }
        // 1) 容错：全角逗号 → 半角；去首尾空格
        String normalized = honor == null ? "" : honor.trim().replace('，', ',');
        // 2) 标准化：按逗号拆分，去重保序，再用半角逗号连接
        if (!normalized.isEmpty()) {
            Set<String> codes = Arrays.stream(normalized.split(","))
                    .map(String::trim)
                    .filter(s -> !s.isEmpty())
                    .collect(Collectors.toCollection(LinkedHashSet::new));
            normalized = String.join(",", codes);
        }
        userMapper.update(null, new LambdaUpdateWrapper<AdminUserDO>()
                .eq(AdminUserDO::getId, id)
                .set(AdminUserDO::getHonor, normalized));
    }
